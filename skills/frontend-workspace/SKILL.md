...
Workspace scope for imports: **`@pietersoudan/…`** (e.g. `@pietersoudan/ui`, `@pietersoudan/ui-lib`).
...
| `scaffold slice <name>` | Service + UI package pair (loosely coupled via domain) |
...
Once a slice exists, wire its exported UI into a Vite frontend with **`scaffold app-module add-ui-module <slice> --app <frontend-app>`**. The CLI bootstraps `src/modules/index.tsx` and keeps module registration deterministic.
...
| `packages/ui-lib` | Composed components, hooks, Storybook. Imports **`@pietersoudan/ui`**. No local Shadcn registry. |
| `packages/ui-<feature>` | Same rules as `ui-lib`; use **`scaffold ui <feature>`**. Depends on `@pietersoudan/ui` (and often `ui-lib`). |
...
- **Storybook:** from the repo root, `bun run storybook` runs every package that defines a `storybook` script in parallel via Turbo (same idea as `bun run dev`). Each UI package uses its own dev port (`packages/ui` → 6006, `packages/ui-lib` → 6007, `packages/ui-*` → stable hash in 6008–6899). Use `bun run build-storybook` for static builds. Shared patterns: `.storybook/main.ts`, Vite alias for `@/…` in Shadcn sources.
